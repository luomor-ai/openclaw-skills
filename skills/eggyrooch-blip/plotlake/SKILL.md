...
- `PLOTLAKE_BASE_URL` — Plotlake API 地址（必填），默认 `https://api.plotlake.com`

## 创建频道

```bash
curl -s -X POST "$PLOTLAKE_BASE_URL/api/open/channels" \
...
# 添加 RSS feed
curl -s -X POST "$PLOTLAKE_BASE_URL/api/open/channels/$CHANNEL_ID/sources" \
  -H "Content-Type: application/json" \
...
```bash
curl -s "$PLOTLAKE_BASE_URL/api/open/channels/$CHANNEL_ID/sources" | jq .
```
...
curl -s -X DELETE "$PLOTLAKE_BASE_URL/api/open/channels/$CHANNEL_ID/sources/$SOURCE_ID"
```
...
```bash
# 最近 1 天
curl -s "$PLOTLAKE_BASE_URL/api/open/channels/$CHANNEL_ID/articles?days=1" | jq .

# 最近 7 天，第 2 页
curl -s "$PLOTLAKE_BASE_URL/api/open/channels/$CHANNEL_ID/articles?days=7&page=2&page_size=50" | jq .
```
...
```bash
curl -s "$PLOTLAKE_BASE_URL/api/open/channels/$CHANNEL_ID/feed"
```
