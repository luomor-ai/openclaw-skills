...
- **语音总结（TTS）**：在技能目录下配置 `.env`。将 `{baseDir}/.env.example` 复制为 `{baseDir}/.env`，填入 `SENSEAUDIO_API_KEY`（[接口密钥](https://senseaudio.cn/platform/api-key)）；可选 `SENSEAUDIO_VOICE_ID`。脚本会自动从该 `.env` 读取。
...
3. **输出**：  
   ...
   - **语音输出**：使用 [SenseAudio API](https://senseaudio.cn/docs/) 将语音稿合成并保存为 mp3（可选播放）。需在技能目录下配置 `.env`：复制 `.env.example` 为 `.env`，填入 `SENSEAUDIO_API_KEY`（在 [接口密钥](https://senseaudio.cn/platform/api-key) 创建）；可选 `SENSEAUDIO_VOICE_ID`。
...
| 脚本 | 用途 |
|------|------|
| `scripts/ensure_mediacrawler.sh` | 首次执行时若不存在 `MediaCrawler` 目录，则从 GitHub 克隆到技能目录下（需在项目根执行）。 |
| `scripts/setup_env.sh` | 创建 conda 环境、安装依赖与 Playwright（需在项目根执行；若 MediaCrawler 不存在会先尝试克隆）。 |
| `scripts/summarize_and_voice.py` | 读取 `MediaCrawler/data/` 下数据，生成 Markdown 报告与简短语音稿，并用 SenseAudio API 合成语音（需 `SENSEAUDIO_API_KEY`）。 |

调用示例（将 `{baseDir}` 换为实际路径；语音需在 `{baseDir}/.env` 中配置 `SENSEAUDIO_API_KEY`，可复制 `.env.example` 为 `.env` 后填写）：
...
- **语音总结**：TTS 使用 SenseAudio API。请在技能目录 `{baseDir}` 下将 `.env.example` 复制为 `.env`，填入 `SENSEAUDIO_API_KEY`（在 [senseaudio.cn 接口密钥](https://senseaudio.cn/platform/api-key) 创建）；可选 `SENSEAUDIO_VOICE_ID`。脚本会自动从该 `.env` 读取。API 文档见 <https://senseaudio.cn/docs/>。
