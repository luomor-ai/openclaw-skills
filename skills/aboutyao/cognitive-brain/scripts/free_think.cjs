...
async function collectThoughtContext() {
  ...
  try {
    ...
    if (files.length >= 2) {
      // 随机选一个过去的文件
      const pastFile = files[Math.floor(Math.random() * (files.length - 1)) + 1];
...
function selectThoughtDirection(context) {
  ...
  if (context.associations.length > 0 && Math.random() < 0.3) {
    const assoc = context.associations[Math.floor(Math.random() * context.associations.length)];
    return {
  ...
  // 如果有过去的思绪，有 30% 概率延续
  if (context.pastThoughts.length > 0 && Math.random() < 0.3) {
    return {
...
async function think() {
  ...
  console.log(`✅ 思考提示已生成: ${THOUGHT_PROMPT_PATH}`);
  ...
  // 6. 输出摘要
  console.log('\n📊 思考上下文:');
  console.log(`   记忆片段: ${context.memoryFragments.length} 个`);
  console.log(`   最近概念: ${context.recentConcepts.length} 个`);
  console.log(`   联想路径: ${context.associations.length} 个`);
  console.log(`   思考方向: ${direction.direction}`);
  console.log(`   情绪: ${state.mood}`);
  
  console.log('\n⚠️  请主 agent 执行: cat ~/.openclaw/workspace/.thought-prompt.md');
  console.log('然后进行真正的自由思考，并将结果写入 thoughts/ 目录。\n');
...
async function main() {
  ...
  switch (action) {
    ...
    case 'status':
      loadState();
      console.log('📊 自由思考状态:');
      console.log(`   总思绪: ${state.totalThoughts}`);
      console.log(`   上次思考: ${state.lastThought ? new Date(state.lastThought).toLocaleString('zh-CN') : '从未'}`);
      console.log(`   当前情绪: ${state.mood}`);
      console.log(`   近期话题: ${state.recentTopics.join(', ') || '无'}`);
      break;
      
    case 'view':
      const thoughtPath = getTodayThoughtPath();
      if (fs.existsSync(thoughtPath)) {
        console.log(fs.readFileSync(thoughtPath, 'utf8'));
      } else {
        console.log('今天还没有思绪');
      }
      break;
