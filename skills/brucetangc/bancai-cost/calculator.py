...
def cleanup_old_excel(output_dir: str, retention_days: int = 35):
    """
    清理旧的 Excel 文件
    
    Args:
        output_dir: 输出目录
        retention_days: 保留天数（默认 35 天）
    """
    ...
    for filename in os.listdir(output_dir):
        # 只清理统计相关的 Excel 文件
        if not (filename.endswith('.xlsx') and 
                'Board material cost statistics' in filename):
            continue
        ...
        if age_days > retention_days:
            ...
            print(f"  🗑️  已删除：{filename} (已 {int(age_days)} 天)")
    
    if deleted_count > 0:
        print(f"  ✅ 清理完成：删除 {deleted_count} 个文件，释放 {total_size/1024:.1f} KB")
    else:
        print(f"  ℹ️  无需清理（保留最近 {retention_days} 天的文件）")
...
class SheetMaterialCalculator:
    ...
    def extract_pdf_data(self, pdf_path: str) -> dict:
        ...
        data = {
            ...
            'zero_parts_warning': []  # Number of Parts 为 0 的警告
        }
        ...
        # 3. 检查 Plan Data 下 Number of Parts 是否为 0
        self._check_zero_parts(data, reader, pdf_path.name)
        
        # 4. 验证提取的板材数量是否和 Material data 一致
        self._verify_material_usage(data, pdf_path.name)
        
        return data
    
    def _check_zero_parts(self, data: dict, reader, filename: str = None):
        """
        检查 Plan Data 下的 Number of Parts 是否为 0
        
        Args:
            data: 包含板材数据的字典
            reader: PDF Reader 对象
            filename: PDF 文件名
        """
        for page_num, page in enumerate(reader.pages):
            text = page.extract_text()
            ...
            for i, line in enumerate(lines):
                ...
                if 'Plan data' in line:
                    in_plan_data = True
                    number_of_parts = None  # 重置
                    continue
                
                # 检测下一个数据块开始（停止 Plan data）
                if in_plan_data and ('Material data' in line or 'Sheet dimension' in line or 'Flat part data' in line):
                    # 检查这个 Plan data 的 Number of Parts 是否为 0
                    if number_of_parts is not None and number_of_parts == 0:
                        data['zero_parts_warning'].append({
                            'file': filename,
                            'page': page_num + 1,
                ...
                # 在 Plan data 块内查找 Number of Parts
                if in_plan_data:
                    # 查找纯数字行（可能是 Number of Parts）
                    if re.match(r'^\d+$', line):
                        num = int(line)
                        # 检查上一行是否包含 "Number of parts" 或 "Number"
                        prev_line = lines[i-1] if i > 0 else ''
                        if 'number' in prev_line.lower() or 'parts' in prev_line.lower():
                            number_of_parts = num
            
            # 检查页面末尾的 Plan data
            if in_plan_data and number_of_parts is not None and number_of_parts == 0:
                data['zero_parts_warning'].append({
                    'file': filename,
                    'page': page_num + 1,
                    'value': number_of_parts
    ...
    def process_folder(self, folder_path: str) -> dict:
        ...
        all_data = {'sheets': {}, 'zero_parts_warnings': []}
        ...
        for pdf_file in sorted(pdf_files, key=lambda x: x.name):
            ...
            # 收集 Number of Parts 为 0 的警告
            if data.get('zero_parts_warning'):
                all_data['zero_parts_warnings'].extend(data['zero_parts_warning'])
        ...
        # 打印所有警告
        if all_data['zero_parts_warnings']:
            print("\n" + "=" * 70)
            print("⚠️  注意：以下 PDF 的 Number of Parts 为 0，请检查！")
            print("=" * 70)
            for warning in all_data['zero_parts_warnings']:
                print(f"📄 {warning['file']} (第{warning['page']}页) - Number of Parts = {warning['value']}")
            print("=" * 70)
            print(f"共 {len(all_data['zero_parts_warnings'])} 个文件需要检查\n")
...
if __name__ == '__main__':
    ...
    print(f"\n🧹 清理旧文件（保留 35 天）...")
    cleanup_old_excel(output_dir, retention_days=35)
    ...
    if output_path and os.path.exists(output_path):
        print(f"\n📤 通过飞书发送文件：{output_path}")
        
        # 复制文件到 media 目录（按命名规则）
        import shutil
        
        # 从文件夹路径提取文件夹名
        folder_name = os.path.basename(folder_path.rstrip('/'))
        
        media_file = f"/home/admin/.openclaw/media/inbound/{folder_name}_Board material cost statistics.xlsx"
        
        shutil.copy(output_path, media_file)
        print(f"✅ 文件已复制到：{media_file}")
        
        # 调用飞书发送命令
        try:
            cmd = [
                'openclaw', 'message', 'send',
                '--target', 'ou_d9120236de5b0f5774c189f625e0fd69',
                '--message', f'📊 板材费用统计\n\n✅ 共 {total_sheets} 张板材，请查收附件',
                '--media', media_file
            ]
