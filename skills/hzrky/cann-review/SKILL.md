...
2. **收集所有未审查的 PR** → 保存到 `.pending-reviews.json`
3. **逐个审查每个 PR** → 完整的代码审查流程
4. **发布审查评论** → 使用 GitCode API
5. **标记为已审查** → 更新 `.review-state.json`
6. **生成汇总报告** → 展示所有审查结果
...
📋 待审查 PR 列表:
  1. cann/runtime#666 - for msprof block dim
     作者: zhaozhixuan
     链接: https://gitcode.com/cann/runtime/merge_requests/666
  ...
