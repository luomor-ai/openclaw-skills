#!/usr/bin/env bash
# text-mining — Text Mining reference tool. Use when working with text mining in finance contexts.
...
show_help() {
    cat << 'HELPEOF'
text-mining v$VERSION — Text Mining Reference Tool

Usage: text-mining <command>
...
## What is Text Mining?
Text Mining (text-mining) is a specialized tool/concept in the finance domain.
It provides essential capabilities for professionals working with text mining.

## Key Concepts
- Core text mining principles and fundamentals
- How text mining fits into the broader finance ecosystem  
- Essential terminology every practitioner should know

## Why Text Mining Matters
Understanding text mining is critical for:
...
## Getting Started
1. Understand the basic text mining concepts
...
cmd_formulas() {
    cat << 'EOF'
# Text Mining — Key Formulas & Calculations
...
cmd_regulations() {
    cat << 'EOF'
# Text Mining — Regulatory Framework
...
cmd_instruments() {
    cat << 'EOF'
# Text Mining — Instruments & Tools Overview

## Primary Instruments
- Core tools used in text mining operations
...
cmd_glossary() {
    cat << 'EOF'
# Text Mining — Key Terms & Definitions

## Core Terminology
- **Text Mining**: The primary subject of this reference
...
cmd_checklist() {
    cat << 'EOF'
# Text Mining — Inspection Checklist
...
case "$CMD" in
    ...
    version|--version|-v) echo "text-mining v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: text-mining help"; exit 1 ;;
esac
