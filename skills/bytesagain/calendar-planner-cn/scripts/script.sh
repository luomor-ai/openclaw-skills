#!/usr/bin/env bash
# calendar-planner-cn - Chinese content creation tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${CALENDAR_PLANNER_CN_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/calendar-planner-cn}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
calendar-planner-cn v$VERSION

Chinese content creation tool

Usage: calendar-planner-cn <command> [args]
...
case "${1:-help}" in
    ...
    help|-h) show_help ;;
    version|-v) echo "calendar-planner-cn v$VERSION" ;;
