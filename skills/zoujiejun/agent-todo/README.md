...
Turn follow-up promises into an execution queue for agents. Instead of only reminding about overdue items, `agent-todo` lets heartbeat pick the next task, claim it, and push the agent to actually do the work.
