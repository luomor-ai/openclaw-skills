---
name: agent-todo
description: Turn follow-up promises into an execution queue for agents. Use when chats or discussions create tasks that should be claimed and executed during heartbeat, with next actions, success criteria, source-aware reports, and completion tracking. 将后续承诺转成 agent 的执行队列；适用于需要在 heartbeat 中认领并推进执行、并按来源生成汇报的任务。
metadata:
