#!/usr/bin/env bash
# warehouse - Data warehouse toolkit — schema design, query opti
# Powered by BytesAgain | bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${XDG_DATA_HOME:-$HOME/.local/share}/warehouse"
mkdir -p "$DATA_DIR"

show_help() {
    echo "Warehouse v$VERSION"
    echo ""
    echo "Usage: warehouse <command> [options]"
    echo ""
    echo "Commands:"
...
cmd_add() {
    local item="${1:?Usage: warehouse add <item>}"
...
cmd_export() {
    local fmt="${1:-json}"
    echo "[warehouse] Exporting as $fmt..."
    case "$fmt" in
        json)
            echo "{"
            echo "  \"tool\": \"warehouse\","
...
case "${1:-help}" in
    ...
    version|-v) echo "warehouse v$VERSION";;
